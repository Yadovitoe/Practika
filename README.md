...
## Лекция 13
Приложение взаимодействует с удалённым API для получения списка пользователей. Для этого используется библиотека Retrofit, которая позволяет удобно осуществлять HTTP-запросы.
В данном приложении UserBloc отвечает за получение пользователей и управление состоянием, а пользовательский интерфейс обновляется в зависимости от состояния блока.
Приложение загружает данные асинхронно и отображает состояние загрузки с помощью CircularProgressIndicator, пока данные не будут получены.
После успешного получения данных приложение отображает список пользователей в виде ListView, где каждый элемент представляет собой карточку с именем и электронной почтой пользователя.
Если возникнет ошибка при получении данных, приложение отображает сообщение об ошибке.

![image](https://github.com/user-attachments/assets/e0f27374-3b8a-4c66-b231-f910fda79e88)

## Лекция 14
Данное приложение представляет из себя счётчик. Его значение может увеличиваться, уменьшаться и обнуляться. Для тестирования исполтьзуются unit тесты и widget тесты.
